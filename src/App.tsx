...
import { FileText, Package, ClipboardList, Camera, Users, Download, Upload, Plus, Trash2, Edit, X, Check, Mail, Percent, Lock, Shield, Wrench, BarChart3, Menu } from 'lucide-react';
...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, string> = { 
    waiting_repair: 'bg-stone-100 text-stone-600 border-stone-200', 
  ...
  return <span className={`text-[10px] px-2 py-0.5 border uppercase tracking-wide ${config[status] || config.waiting_repair}`}>{labels[status] || status}</span>;
}
...
function WorkOrdersPage({ appData, setAppData, currentUser, setModal }: { appData: AppData; setAppData: (d: AppData) => void; currentUser: User; setModal: (m: any) => void }) {
  ...
  const filtered = appData.workOrders
    .filter(wo => showArchived ? wo.isArchived : !wo.isArchived)
    .filter(wo => wo.clientName.toLowerCase().includes(filter.toLowerCase()) || wo.number.toLowerCase().includes(filter.toLowerCase()));
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <h2 className="text-lg font-bold text-stone-900 uppercase tracking-wide">Заказ-наряды</h2>
        <div className="flex gap-2">
          <button onClick={() => setShowArchived(!showArchived)} className="flex items-center gap-2 px-4 py-2.5 bg-stone-100 text-stone-700 border border-stone-300 text-xs uppercase tracking-wider hover:bg-stone-200">
      ...
      <div className="space-y-3">
            ...
            <div key={wo.id} className="bg-white border border-stone-200 p-4 lg:p-5">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                <div className="flex-1">
                  ...
                  <div className="text-xs text-stone-500 mt-0.5">{wo.carBrand} {wo.carModel} • Пробег: {wo.mileage} км{wo.vin && ` • VIN: ${wo.vin}`}</div>
                </div>
        ...
        {filtered.length === 0 && <div className="bg-white border border-stone-200 p-12 text-center"><p className="text-xs text-stone-400 uppercase tracking-wider">{showArchived ? 'Архив пуст' : 'Нет заказ-нарядов'}</p></div>}
      </div>
    </div>
  );
}
        ...
        <table className="w-full text-sm min-w-[700px]">
          <thead className="bg-stone-50 border-b border-stone-200">
            <tr>
              ...
              <th className="text-right px-4 py-3 text-[10px] font-semibold text-stone-500 uppercase tracking-wider">Цена закупа</th>
              <th className="text-right px-4 py-3 text-[10px] font-semibold text-stone-500 uppercase tracking-wider">Цена продажи</th>
          ...
          <tbody>
            {filtered.map(p => (
              <tr key={p.id} className="border-b border-stone-100 hover:bg-stone-50">
                ...
                <td className="px-4 py-3 text-right text-stone-600">{p.purchasePrice.toFixed(2)} ₽</td>
        ...
        <table className="w-full text-sm min-w-[600px]">
          <thead className="bg-stone-50 border-b border-stone-200">
            <tr>
              ...
              <th className="text-right px-4 py-3 text-[10px] font-semibold text-stone-500 uppercase tracking-wider">Нормо-час (₽)</th>
              <th className="text-right px-4 py-3 text-[10px] font-semibold text-stone-500 uppercase tracking-wider">Время (ч)</th>
              <th className="text-right px-4 py-3 text-[10px] font-semibold text-stone-500 uppercase tracking-wider">Итого</th>
              <th className="text-right px-4 py-3 text-[10px] font-semibold text-stone-500 uppercase tracking-wider">Действия</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map(w => (
              <tr key={w.id} className="border-b border-stone-100 hover:bg-stone-50">
                ...
                <td className="px-4 py-3 text-right text-stone-600">{w.ratePerHour.toFixed(2)} ₽</td>
                <td className="px-4 py-3 text-right text-stone-600">{w.duration} ч</td>
...
function InspectionsPage({ appData, setAppData, currentUser, setModal }: { appData: AppData; setAppData: (d: AppData) => void; currentUser: User; setModal: (m: any) => void }) {
  const [showArchived, setShowArchived] = useState(false);
  const inspections = showArchived ? appData.inspections.filter(i => i.isArchived) : appData.inspections.filter(i => !i.isArchived);
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        ...
        {inspections.length === 0 && <div className="bg-white border border-stone-200 p-12 text-center"><p className="text-xs text-stone-400 uppercase tracking-wider">{showArchived ? 'Архив пуст' : 'Нет осмотров'}</p></div>}
      </div>
    </div>
  );
}
...
function ActsPage({ appData, setAppData, currentUser, setModal }: { appData: AppData; setAppData: (d: AppData) => void; currentUser: User; setModal: (m: any) => void }) {
  const [showArchived, setShowArchived] = useState(false);
  const acts = showArchived ? appData.acceptanceActs.filter(a => a.isArchived) : appData.acceptanceActs.filter(a => !a.isArchived);
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        ...
        {acts.length === 0 && <div className="bg-white border border-stone-200 p-12 text-center"><p className="text-xs text-stone-400 uppercase tracking-wider">{showArchived ? 'Архив пуст' : 'Нет актов'}</p></div>}
      </div>
    </div>
  );
}
          ...
          <div className="p-4 space-y-2">
            {selectedClient.cars.map(car => (
              <div key={car.id} className="flex items-center justify-between py-2 border-b border-stone-100 last:border-0">
                <div><span className="text-sm font-medium text-stone-800">{car.brand} {car.model}</span><span className="text-xs text-stone-500 ml-2">({car.year})</span>{car.vin && <span className="text-[10px] text-stone-400 ml-2 font-mono">VIN: {car.vin}</span>}</div>
...
function AnalyticsPage({ appData }: { appData: AppData }) {
  ...
  const statusData = [
    { name: 'Ожидание ремонта', value: appData.workOrders.filter(w => w.status === 'waiting_repair').length },
    { name: 'В работе', value: appData.workOrders.filter(w => w.status === 'in_progress').length },
    { name: 'Согласование', value: appData.workOrders.filter(w => w.status === 'approval').length },
    { name: 'Ожидание запчастей', value: appData.workOrders.filter(w => w.status === 'waiting_parts').length },
    { name: 'Выполнен', value: appData.workOrders.filter(w => w.status === 'completed').length },
    { name: 'Закрыт', value: appData.workOrders.filter(w => w.status === 'closed').length },
  ].filter(d => d.value > 0);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white border border-stone-200 p-5">
          <h3 className="text-[10px] font-semibold text-stone-500 uppercase tracking-wider mb-4">ТОП-5 услуг</h3>
          {(() => {
            const workStats: Record<string, { name: string; count: number; revenue: number }> = {};
            appData.workOrders.forEach(wo => {
              wo.items.filter(i => i.type === 'work').forEach(item => {
                if (!workStats[item.name]) workStats[item.name] = { name: item.name, count: 0, revenue: 0 };
                workStats[item.name].count += item.quantity;
                workStats[item.name].revenue += item.quantity * item.price;
              });
            });
            const top5 = Object.values(workStats).sort((a, b) => b.count - a.count).slice(0, 5);
            return top5.length > 0 ? (
              <div className="space-y-2">
                {top5.map((w, i) => (
                  <div key={w.name} className="flex items-center justify-between py-2 border-b border-stone-100 last:border-0">
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-bold text-stone-400 w-4">{i + 1}</span>
                      <span className="text-xs text-stone-800 font-medium">{w.name}</span>
                    </div>
                    <div className="text-right">
                      <div className="text-xs font-bold text-stone-900">{w.count} раз</div>
                      <div className="text-[10px] text-stone-500">{w.revenue.toFixed(0)} ₽</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="bg-white border border-stone-200 p-5">
          <h3 className="text-[10px] font-semibold text-stone-500 uppercase tracking-wider mb-4">ТОП-5 запчастей</h3>
          {(() => {
            const partStats: Record<string, { name: string; count: number; revenue: number }> = {};
            appData.workOrders.forEach(wo => {
              wo.items.filter(i => i.type === 'part').forEach(item => {
                if (!partStats[item.name]) partStats[item.name] = { name: item.name, count: 0, revenue: 0 };
                partStats[item.name].count += item.quantity;
                partStats[item.name].revenue += item.quantity * item.price;
              });
            });
            const top5 = Object.values(partStats).sort((a, b) => b.count - a.count).slice(0, 5);
            return top5.length > 0 ? (
              <div className="space-y-2">
                {top5.map((p, i) => (
                  <div key={p.name} className="flex items-center justify-between py-2 border-b border-stone-100 last:border-0">
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-bold text-stone-400 w-4">{i + 1}</span>
                      <span className="text-xs text-stone-800 font-medium">{p.name}</span>
                    </div>
                    <div className="text-right">
                      <div className="text-xs font-bold text-stone-900">{p.count} шт</div>
                      <div className="text-[10px] text-stone-500">{p.revenue.toFixed(0)} ₽</div>
                    </div>
                  </div>
                ))}
              </div>
...
function WorkOrderForm({ appData, setAppData, currentUser, onClose, order }: { appData: AppData; setAppData: (d: AppData) => void; currentUser: User; onClose: () => void; order?: WorkOrder }) {
  const [form, setForm] = useState({
    ...
    status: order?.status || 'waiting_repair' as WorkOrder['status'],
    notes: order?.notes || '',
  });
  ...
  const handleSubmit = () => {
    ...
    const isArchived = form.status === 'closed';
    const closedAt = form.status === 'closed' ? new Date().toISOString() : order?.closedAt;
    
    if (order) {
      const updated = updateWorkOrder(appData, order.id, { ...form, totalAmount, isArchived, closedAt });
      
      if (form.status === 'closed') {
        if (order.inspectionId) {
          updated.inspections = updated.inspections.map(i => i.id === order.inspectionId ? { ...i, isArchived: true } : i);
        }
        if (order.acceptanceActId) {
          updated.acceptanceActs = updated.acceptanceActs.map(a => a.id === order.acceptanceActId ? { ...a, isArchived: true } : a);
        }
      }
      
      setAppData(updated);
    } else {
      setAppData(addWorkOrder(appData, { ...form, totalAmount, isArchived, closedAt, createdBy: currentUser.id }));
    }
    onClose();
  };
  ...
  return (
    <div className="p-4 lg:p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        ...
        <div className="sm:col-span-2"><label className="text-[10px] font-semibold text-stone-500 uppercase tracking-wider mb-1 block">VIN</label><input type="text" value={form.vin} onChange={e => setForm({ ...form, vin: e.target.value })} className="w-full px-3 py-2 border border-stone-300 text-sm focus:border-stone-500 focus:outline-none font-mono" placeholder="WBA3A5C55CF256789" /></div>
        <div className="sm:col-span-2"><label className="text-[10px] font-semibold text-stone-500 uppercase tracking-wider mb-1 block">Статус</label><select value={form.status} onChange={e => setForm({ ...form, status: e.target.value as any })} className="w-full px-3 py-2 border border-stone-300 text-sm bg-white focus:border-stone-500 focus:outline-none"><option value="waiting_repair">Ожидание ремонта</option><option value="in_progress">В работе</option><option value="approval">Согласование</option><option value="waiting_parts">Ожидание запчастей</option><option value="completed">Выполнен</option><option value="closed">Закрыт</option></select></div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {appData.workItems.length > 0 && (
              <div className="relative">
                <input type="text" placeholder="Поиск работы..." value={workSearch} onChange={e => setWorkSearch(e.target.value)} className="w-40 px-2 py-1.5 text-[11px] border border-stone-300 focus:border-stone-500 focus:outline-none" />
                {workSearch && (
                  <div className="absolute left-0 top-full mt-1 w-72 bg-white border border-stone-300 shadow-lg z-10 max-h-60 overflow-y-auto">
                    {appData.workItems.filter(w => w.name.toLowerCase().includes(workSearch.toLowerCase())).map(w => <button key={w.id} onClick={() => { addWorkFromCatalog(w); setWorkSearch(''); }} className="w-full text-left px-3 py-2 text-xs hover:bg-stone-50 border-b border-stone-100"><span className="font-medium">{w.name}</span> — <span className="font-semibold">{w.price}₽</span></button>)}
                    {appData.workItems.filter(w => w.name.toLowerCase().includes(workSearch.toLowerCase())).length === 0 && <div className="px-3 py-2 text-xs text-stone-400">Не найдено</div>}
                  </div>
                )}
              </div>
            )}
            {appData.spareParts.length > 0 && (
              <div className="relative">
                <input type="text" placeholder="Поиск запчасти..." value={partSearch} onChange={e => setPartSearch(e.target.value)} className="w-40 px-2 py-1.5 text-[11px] border border-stone-300 focus:border-stone-500 focus:outline-none" />
                {partSearch && (
                  <div className="absolute left-0 top-full mt-1 w-72 bg-white border border-stone-300 shadow-lg z-10 max-h-60 overflow-y-auto">
                    {appData.spareParts.filter(p => p.name.toLowerCase().includes(partSearch.toLowerCase()) || p.article.toLowerCase().includes(partSearch.toLowerCase())).map(p => <button key={p.id} onClick={() => { addFromCatalog(p); setPartSearch(''); }} className="w-full text-left px-3 py-2 text-xs hover:bg-stone-50 border-b border-stone-100"><span className="font-medium">{p.name}</span> <span className="text-stone-400 font-mono">({p.article})</span> — <span className="font-semibold">{p.price}₽</span></button>)}
                    {appData.spareParts.filter(p => p.name.toLowerCase().includes(partSearch.toLowerCase()) || p.article.toLowerCase().includes(partSearch.toLowerCase())).length === 0 && <div className="px-3 py-2 text-xs text-stone-400">Не найдено</div>}
                  </div>
                )}
              </div>
            )}
...
function PartForm({ appData, setAppData, onClose, part }: { appData: AppData; setAppData: (d: AppData) => void; onClose: () => void; part?: SparePart }) {
  const [form, setForm] = useState({ article: part?.article || '', name: part?.name || '', brand: part?.brand || '', price: part?.price || 0, purchasePrice: part?.purchasePrice || 0, quantity: part?.quantity || 0, category: part?.category || '', supplier: part?.supplier || '' });
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
        ...
        <div><label className="text-[10px] font-semibold text-stone-500 uppercase tracking-wider mb-1 block">Цена продажи *</label><input type="number" min="0" value={form.price} onChange={e => setForm({ ...form, price: Number(e.target.value) })} className="w-full px-3 py-2 border border-stone-300 text-sm focus:border-stone-500 focus:outline-none" /></div>
        <div><label className="text-[10px] font-semibold text-stone-500 uppercase tracking-wider mb-1 block">Цена закупа</label><input type="number" min="0" value={form.purchasePrice} onChange={e => setForm({ ...form, purchasePrice: Number(e.target.value) })} className="w-full px-3 py-2 border border-stone-300 text-sm focus:border-stone-500 focus:outline-none" /></div>
    ...
    try {
      ...
      if (format === 'json') parts = JSON.parse(textData);
        ...
        parts = lines.slice(1).map(line => {
          ...
          return { article: obj['артикул'] || obj['article'] || '', name: obj['название'] || obj['name'] || '', brand: obj['бренд'] || obj['brand'] || '', price: parseFloat(obj['цена'] || obj['price'] || '0'), purchasePrice: parseFloat(obj['цена закупа'] || obj['purchaseprice'] || obj['purchase_price'] || '0'), quantity: parseInt(obj['количество'] || obj['quantity'] || '0'), category: obj['категория'] || obj['category'] || '', supplier: obj['поставщик'] || obj['supplier'] || '' };
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-4">
        ...
        <textarea value={textData} onChange={e => setTextData(e.target.value)} placeholder="артикул;название;бренд;цена;цена закупа;количество" className="w-full px-3 py-2 border border-stone-300 text-xs h-40 font-mono focus:border-stone-500 focus:outline-none resize-none" />
      </div>
      <div className="flex justify-end gap-3 pt-4 border-t border-stone-200">
        <button onClick={onClose} className="px-5 py-2.5 text-xs text-stone-700 bg-stone-100 border border-stone-300 hover:bg-stone-200 uppercase tracking-wide">Отмена</button>
...
function WorkItemForm({ appData, setAppData, onClose, workItem }: { appData: AppData; setAppData: (d: AppData) => void; onClose: () => void; workItem?: WorkItem }) {
  const [form, setForm] = useState({ name: workItem?.name || '', category: workItem?.category || '', ratePerHour: workItem?.ratePerHour || 0, duration: workItem?.duration || 1, description: workItem?.description || '' });
  const calculatedPrice = form.ratePerHour * form.duration;
  const handleSubmit = () => { 
    const finalForm = { ...form, price: calculatedPrice };
    if (workItem) setAppData(updateWorkItem(appData, workItem.id, finalForm)); 
    else setAppData(addWorkItem(appData, finalForm)); 
    onClose(); 
  };
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
        ...
        <div><label className="text-[10px] font-semibold text-stone-500 uppercase tracking-wider mb-1 block">Категория</label><input type="text" value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} className="w-full px-3 py-2 border border-stone-300 text-sm focus:border-stone-500 focus:outline-none" /></div>
        <div><label className="text-[10px] font-semibold text-stone-500 uppercase tracking-wider mb-1 block">Стоимость нормо-часа (₽) *</label><input type="number" min="0" value={form.ratePerHour} onChange={e => setForm({ ...form, ratePerHour: Number(e.target.value) })} className="w-full px-3 py-2 border border-stone-300 text-sm focus:border-stone-500 focus:outline-none" /></div>
        <div><label className="text-[10px] font-semibold text-stone-500 uppercase tracking-wider mb-1 block">Время (часы) *</label><input type="number" min="0" step="0.5" value={form.duration} onChange={e => setForm({ ...form, duration: Number(e.target.value) })} className="w-full px-3 py-2 border border-stone-300 text-sm focus:border-stone-500 focus:outline-none" /></div>
        <div><label className="text-[10px] font-semibold text-stone-500 uppercase tracking-wider mb-1 block">Итого стоимость (авто)</label><div className="w-full px-3 py-2 border border-stone-200 bg-stone-50 text-sm font-bold text-stone-900">{calculatedPrice.toFixed(2)} ₽</div></div>
      </div>
      <div className="flex justify-end gap-3 pt-4 border-t border-stone-200">
        <button onClick={onClose} className="px-5 py-2.5 text-xs text-stone-700 bg-stone-100 border border-stone-300 hover:bg-stone-200 uppercase tracking-wide">Отмена</button>
        <button onClick={handleSubmit} disabled={!form.name || form.ratePerHour <= 0} className="px-5 py-2.5 text-xs text-white bg-stone-900 hover:bg-stone-800 uppercase tracking-wide disabled:opacity-50">Сохранить</button>
      </div>
    </div>
  );
}
          ...
          {form.cars.map((car, idx) => (
            <div key={car.id} className="border border-stone-200 p-3 bg-stone-50">
              <div className="grid grid-cols-2 gap-2">
                ...
                <input type="text" placeholder="VIN" value={car.vin} onChange={e => updateCar(idx, { vin: e.target.value })} className="px-2 py-1 border border-stone-300 text-xs focus:border-stone-500 focus:outline-none font-mono" />
