import { AppData, User, SparePart, WorkItem, WorkOrder, Inspection, AcceptanceAct, Client } from '../types';
...
export function importSpareParts(state: AppData, parts: any[]): AppData {
  const newParts = parts.map(p => ({ ...p, purchasePrice: p.purchasePrice || 0, id: uuidv4(), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }));
